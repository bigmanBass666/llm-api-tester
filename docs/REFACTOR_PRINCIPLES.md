...
# ~~src/nvidia_client.py~~ (已于 v2.0 删除): setup_ssl_certificates()
# ~~src/zhipu_client.py~~ (已于 v2.0 删除): setup_ssl_certificates()
...
def register_platform(platform_name: str):
    ...
    def decorator(cls):
        ...
        registry.register(platform_name, cls)
        return cls
    return decorator
...
**本次案例示例**：
- `BaseClient` (~~[src/base_client.py](../src/base_client.py)~~ 已于 v2.0 删除) vs `BasePlatformClient` ([platforms/base/base_client.py](../platforms/base/base_client.py))
...
```
~~src/base_client.py~~ (已于 v2.0 删除):class BaseClient:
...
**本次案例示例**：
- `src/zhipu_client.ZhipuClient` (~~已于 v2.0 删除~~) vs `platforms/zhipu/client.ZhipuClient`
...
```
~~src/nvidia_client.py~~ (已于 v2.0 删除)-    def __init__(self):
~~src/nvidia_client.py~~ (已于 v2.0 删除):        self._setup_ssl_certificates()
~~src/nvidia_client.py~~ (已于 v2.0 删除)-        
~~src/zhipu_client.py~~ (已于 v2.0 删除)-    def __init__(self):
~~src/zhipu_client.py~~ (已于 v2.0 删除):        self._setup_ssl_certificates()
--
...
**本次案例示例**：
- `BaseClient` 已废弃但被 `~~src/zhipu_client.py~~` (已于 v2.0 删除) 和 `~~src/nvidia_client.py~~` (已于 v2.0 删除) 使用
...
2. **循环依赖检测**
   ...
   本案检测结果：
   ```
   src/platform_registry.py → ~~src/base_client.py~~ (已于 v2.0 删除) → src/models.py
   结论: 无循环依赖 ✅ 良好
   ```
...
1. 删除废弃文件：
   ```bash
   rm src/base_client.py      # ~~已废弃~~ (已于 v2.0 删除)
   rm src/zhipu_client.py     # ~~重复实现~~ (已于 v2.0 删除)
   rm src/nvidia_client.py    # ~~已迁移~~ (已于 v2.0 删除)
   ```
...
class NewClient(BasePlatformClient):
    ...
    def chat(self, model: str, messages, **kwargs) -> str:
        # ... 新实现 ...
        pass
    
    async def chat_stream(self, model: str, messages, **kwargs):
        # ... 新实现 ...
        async def gen():
            yield ""
        return gen()
...
def get_client(platform: str):
    if USE_NEW_CLIENT:
        from src.platform_registry import registry
        return registry.get_client_class(platform)()
    else:
        from src.platform_registry import registry
        return registry.get_client_class(platform)()
```
...
def create_client(platform: str, api_key: str = None, **kwargs):
    ...
    if platform not in registry.get_registered_platforms():
        raise ValueError(f"Unknown platform: {platform}. Available: {list(registry.get_registered_platforms().keys())}")
    ...
    return registry.create_client(platform, config)
...
```
2026-03: 项目启动
├── 创建 ~~src/base_client.py~~ (已于 v2.0 删除) 作为基础客户端
├── 实现 ~~src/nvidia_client.py~~ (已于 v2.0 删除) 和 ~~src/zhipu_client.py~~ (已于 v2.0 删除)
└── 一切正常，结构清晰
...
```bash
$ grep -r "class ZhipuClient" --include="*.py"
~~src/zhipu_client.py~~ (已于 v2.0 删除):26:class ZhipuClient(BaseClient):     # ~~旧版~~ (已删除)
...
**Step 3: 量化代码重复**
```bash
$ wc -l ~~src/zhipu_client.py~~ (已于 v2.0 删除) platforms/zhipu/client.py
~~204 src/zhipu_client.py~~ (已于 v2.0 删除)       # ~~旧版~~ (已删除)
...
Day 1: 发现 SSL 证书问题
    ↓
Day 2: 修复 ~~src/nvidia_client.py~~ (已于 v2.0 删除) 中的 SSL 设置
    ↓
Day 3: 测试通过，部署
    ↓
...
Day 6: 又修了一处
    ↓
Day 7: 又发现 ~~src/zhipu_client.py~~ (已于 v2.0 删除) 也需要修...
    ↓
结论: 同一个问题，修了 3 次！
```
...
from src.models import ChatMessage, ModelInfo
...
```python
# ~~src/base_client.py~~ (已于 v2.0 删除) (修改后)
"""
...
if "platforms" not in sys.modules:
    ...
    from src.models import ChatMessage, ChatResponse, ModelInfo
